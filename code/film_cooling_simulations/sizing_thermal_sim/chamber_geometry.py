def get(Rt_SI, ER, CR, Lstar_target_mm):
    """
    Build a simple chamber + Rao bell nozzle contour and return stitched geometry.

    Computes:
      • Convergent (arc–cone–arc), cylinder (sized to meet L*), and divergent (Rao bell).
      • Stitches sections axially and returns full x–y wall profile.

    Parameters
    ----------
    Rt_SI : float
        Throat radius in meters (m). Internally converted to mm.
    ER : float
        Expansion ratio, Ae/At (unitless).
    CR : float
        Contraction ratio, Ac/At (unitless).
    Lstar_target_mm : float
        Target characteristic length L* in millimeters (mm). Used to size cylinder.

    Returns
    -------
    x_full : ndarray
        Axial coordinates of full contour in millimeters (mm).
    y_full : ndarray
        Radius coordinates of full contour in millimeters (mm).
    Rc_mm : float
        Chamber radius (mm).
    Rt_mm : float
        Throat radius (mm).
    Re_mm : float
        Exit radius (mm).
    throat_node_fwd : int
        Index of the throat in the returned arrays.

    Notes
    -----
    • All returned geometry is in mm. Internal helper volumes are in mm³.
    • Convergent uses two fillets (R2 upstream, R1 near throat) with a 30° cone between.
    • Divergent uses a shortened Rao bell (~80% of 15° cone length) with 24° → 12° wall angles.
    """
    import math
    import numpy as np
    def rao_nozzle(Rt, ER):
        """
        ...
        nozzle_length = 0.8 * ((math.sqrt(ER) - 1) * Rt / math.tan(math.radians(15)))
    ...
    Rt_mm = Rt_SI * 1e3
    Re_mm = math.sqrt(ER) * Rt_mm
    Rc_mm = math.sqrt(CR) * Rt_mm

    # Bell (Rao) nozzle
    xn, yn, vn_mm3, Rn_mm, theta_n, theta_e = rao_nozzle(Rt_mm, ER)
    ...
    xc   = xc + np.max(xcyl)
    xn   = xn + np.max(xc)
    x_full = np.concatenate((xcyl, xc[:-1], xn))
    y_full = np.concatenate((ycyl, yc[:-1], yn))

    # Throat index in forward arrays
    throat_node_fwd = np.argmin(y_full)
    
    return(x_full, y_full, Rc_mm, Rt_mm, Re_mm, throat_node_fwd)